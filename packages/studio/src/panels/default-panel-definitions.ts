...
const panels: Record<string, Panel> = {
  ...
  error: { eyebrow: "04 / MÉTRICAS", title: "Curva de erro", body: `<div class="visual" style="height:120px"><canvas class="error-chart" data-studio-field="error-chart" aria-label="Curva de erro RMS"></canvas></div><p data-studio-field="error-summary">Aguardando amostras do runtime.</p><small data-error-source>RMS: aguardando medição</small>` },
  live: { eyebrow: "PALCO CENTRAL", title: "Live 3D / Face overlay", body: `<div id="stage" class="visual"><canvas class="studio-live-canvas" aria-label="Canvas Live 3D"></canvas><canvas class="studio-landmark-canvas studio-runtime-hidden" aria-label="Malha facial 3D"></canvas><span class="studio-runtime-caption" data-studio-active-text="rosto ciano · GLB âmbar · anchors verdes" style="position:absolute;bottom:10px;left:12px;color:#6f89a6">rosto ciano · GLB âmbar · anchors verdes</span></div>` },
  viewports: { eyebrow: "GEOMETRIA", title: "Viewports 3D", body: `${renderViewportGrid(["FRONT", "TOP", "LEFT", "RIGHT"].map((label) => ({ label, body: `<div class="visual"><canvas class="viewport-canvas" data-viewport="${label}" aria-label="Projeção facial ${label}"></canvas></div><small data-viewport-caption>rosto 3D · GLB aguardando carregamento</small>` })))}<div class="reconstruction-progress-wrap"><div><small data-reconstruction-progress-label id="reconstruction-progress-label">0% · aguardando captura</small><small>Gire o rosto para completar a profundidade</small></div><progress id="reconstruction-progress" max="100" value="0" aria-label="Progresso da reconstrução 3D"></progress></div><small class="reconstruction-status" data-reconstruction-status>Aguardando leitura facial</small><small class="reconstruction-legend"><span class="legend-observed">● leitura real</span><span class="legend-estimated">● preenchimento estimado</span><span class="legend-glasses">○ óculos GLB wireframe</span></small>` },
...
function updateRuntimePresentation(element: HTMLElement, snapshot: AuditSnapshot): void {
  ...
  if (snapshot.mode === "connected") {
    ...
    const face = snapshot.face as { rmsError?: number; rmsErrorEstimated?: boolean; interpupilar?: number; bbox?: { width?: number; height?: number } } | undefined;
    ...
    const errorSource = element.querySelector<HTMLElement>("[data-error-source]");
    if (errorSource) errorSource.textContent = face?.rmsError === undefined ? "RMS: aguardando medição" : face.rmsErrorEstimated ? "RMS estimado · confiança/estabilidade" : "RMS medido pelo runtime";
