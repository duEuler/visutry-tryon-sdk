...
const panels: Record<string, Panel> = {
  ...
  viewports: { eyebrow: "GEOMETRIA", title: "Viewports 3D", body: `${renderViewportGrid(["FRONT", "TOP", "LEFT", "RIGHT"].map((label) => ({ label, body: `<div class="visual"><canvas class="viewport-canvas" data-viewport="${label}" aria-label="Projeção facial ${label}"></canvas></div><small>rosto 3D · óculos wireframe</small>` })))}<small class="reconstruction-status" data-reconstruction-status>Ao vivo · inicie uma reconstrução para congelar</small><small class="reconstruction-legend"><span class="legend-observed">● leitura real</span><span class="legend-estimated">● preenchimento estimado</span><span class="legend-glasses">○ GLB wireframe</span></small>` },
