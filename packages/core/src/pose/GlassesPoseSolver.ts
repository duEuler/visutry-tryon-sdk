import type {
  ...
  GlassesFittingConfig,
  GlassesPose,
  GlassesPoseSolverInput,
...
export const DEFAULT_FITTING_CONFIG: GlassesFittingConfig = {
  scaleMultiplier: 1,
...
export class GlassesPoseSolver {
  ...
  private fitMetricRW(
    sem: NormalizedFaceResult["landmarks"]["semantic"],
    fitBy: NonNullable<GlassesFittingConfig["fitBy"]>,
    aspect: number,
  ): number {
    const toRW = (p?: Point3D) => (p ? CoordinateSystem.normalizedToRenderWorld(p, aspect) : undefined);
    let a: Point3D | undefined;
  ...
  private modelFrameWidthMm(asset: GlassesAssetManifest): number {
    const unitFactor =
      asset.coordinateSystem.unit === "millimeter"
        ? 1
  ...
  private assessVisibility(face: NormalizedFaceResult): {
    visible: boolean;
    confidence: number;
    reason?: string;
  } {
  ...
  private deriveAspect(face: NormalizedFaceResult): number {
 ...
 */
export function decomposeMatrixToEuler(matrix: number[]): { x: number; y: number; z: number } {
